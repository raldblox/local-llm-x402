...
export default function ChatContainer({ mode, role }: { mode: LandingMode; role?: DemoRole }) {
  ...
  const hostOnline =
    mode === 'demo' && identityRole === 'host' ? hostOnlineLocal : Boolean(hostState?.modelConnected)
  const currentAddr =
    identityRole === 'host' ? identities.hostAddr ?? identities.guestAddr : identities.guestAddr

  const showGuestLink = mode === 'demo'
  const canBecomeHost = !hostOnline && (!currentAddr || identityRole === 'guest')
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      const seed =
        identityRole === 'guest' && currentAddr === identities.guestAddr
          ? DEFAULT_GUEST_SEED
          : 0
  ...
  useInterval(fetchBalance, mode === 'demo' && identityRole === 'host' ? null : currentAddr && (hostOnline || hasInteracted) ? 4000 : null)

  const handleSend = async () => {
    if (!chatInput.trim() || !currentAddr || sending) return
    if (!hostOnline) {
      setChatInput('')
      return
    }
    ...
    try {
      if (mode === 'demo' && identityRole === 'host') {
        ...
        const root = lmStudioUrl.trim().replace(/\/+$/, '').replace(/\/api\/v1$/i, '').replace(/\/v1$/i, '')
      ...
      const data = await response.json()
      const nextMessages = [data?.prompt, data?.response, data?.system].filter(Boolean) as Message[]
  ...
  const handleDetectModels = async () => {
    ...
    try {
      ...
      const directFetch = async () => {
        const root = trimmedUrl.replace(/\/+$/, '').replace(/\/api\/v1$/i, '').replace(/\/v1$/i, '')
  ...
  const navLeftLabel = hostOnline ? 'Host: Online' : 'Connect LM Studio'
      ...
      <header className="border-b border-border/60">
        <div className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
          <div className="flex items-center gap-2">
            <Button variant={hostOnline ? 'secondary' : 'default'} onClick={() => setModalOpen(true)}>
              {navLeftLabel}
            </Button>
            <span className="hidden text-xs text-muted-foreground md:inline">{modelLabel}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="px-3 py-1 text-xs">
              {balanceLabel}: {formatUsdc(balance.microUsdc)}
            </Badge>
          </div>
        </div>
      </header>
        ...
        <div className="flex flex-1 flex-col gap-4 rounded-2xl border border-border bg-muted/20 p-4">
          {bannerVisible ? (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-background/80 px-4 py-3 text-sm text-muted-foreground">
              <span>No model connected. Be the host to earn.</span>
              {canBecomeHost ? (
        ...
        {mode === 'demo' ? (
          <div className="mt-4 grid gap-3 text-xs text-muted-foreground md:grid-cols-2">
            <div className="rounded-lg border border-border bg-muted/20 p-3">
              <p className="font-semibold text-foreground">Debug: Host State</p>
              <pre className="mt-2 whitespace-pre-wrap break-words text-[11px]">
                {hostState ? JSON.stringify(hostState, null, 2) : 'No host connected'}
              </pre>
            </div>
            <div className="rounded-lg border border-border bg-muted/20 p-3">
              <p className="font-semibold text-foreground">Debug: Identity</p>
              <pre className="mt-2 whitespace-pre-wrap break-words text-[11px]">
                {JSON.stringify({ roomId, currentAddr, identities }, null, 2)}
              </pre>
            </div>
          </div>
        ) : null}
