...
const createAgentServer = async () => {
  ...
  const handleModels = async (request, reply) => {
    ...
    try {
      const response = await fetch(`${target}/api/v1/models`, {
  ...
  fastify.get('/api/v1/models', handleModels)

  const handleChat = async (request, reply, isStatefulChat) => {
    const overrides = [
    ...
    try {
      const endpoint = isStatefulChat ? `${target}/api/v1/chat` : `${target}/v1/chat/completions`
  ...
  fastify.post('/v1/chat/completions', (request, reply) => handleChat(request, reply, false))
  fastify.post('/api/v1/chat', (request, reply) => handleChat(request, reply, true))
